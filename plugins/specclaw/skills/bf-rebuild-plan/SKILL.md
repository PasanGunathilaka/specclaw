---
description: Read the analysis documents and write an ordered `.specclaw/analysis/rebuild-backlog.md` — the app decomposed into individually-proposable items grouped by MOD-### module, each with its acceptance basis, Gate and Verification state, plus quality-remediation items where /specclaw:bf-quality found hotspots. Run in the legacy repo; `--refresh` to recompute.
---
